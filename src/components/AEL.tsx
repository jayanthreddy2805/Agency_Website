...
export default function AEL() {
  ...
  const [currentUser, setCurrentUser] = useState<AELUser | null>(null);
          ...
          try {
            ...
            if (
              ...
              if (!messageAdded) {
                setMessages((prev) => [
                  ...prev,
                  { role: "assistant", content: displayText, hasEscalation },
                ]);
                messageAdded = true;
              } else {
                setMessages((prev) => {
  ...
  const sandParticles = useMemo(
    () =>
      Array.from({ length: 140 }, (_, i) => ({
        ...
        color:
          i % 4 === 0 ? "#6b6b6b" : i % 4 === 1 ? "#f5f2ed" : i % 4 === 2 ? "#9a9090" : "#d0ccc6",
      })),
    []
  );
  ...
  const getGreeting = () => {
    if (currentUser) {
      return `Hey ${currentUser.username}! What do you need?`;
    }
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes flashFill { 0%{opacity:0;transform:scaleX(0);transform-origin:right}100%{opacity:1;transform:scaleX(1);transform-origin:right} }
        @keyframes boxPop { 0%{transform:scale(0.55);opacity:0}60%{transform:scale(1.05);opacity:1}80%{transform:scale(0.98)}100%{transform:scale(1);opacity:1} }
        @keyframes fadeUpIn { from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)} }
        @keyframes blink { 0%,100%{opacity:1}50%{opacity:0} }
        @keyframes bgSlideLeft { 0%{transform:translateX(0);opacity:1}100%{transform:translateX(-100%);opacity:0.2} }
        @keyframes sandLeft { 0%{transform:translate(0,0) scale(1);opacity:var(--op);filter:blur(0)}35%{opacity:var(--op);filter:blur(0)}100%{transform:translate(var(--dx),var(--dy)) scale(0.2);opacity:0;filter:blur(3px)} }
        @keyframes cardDriftLeft { 0%{transform:translateX(0) scale(1);opacity:1;filter:blur(0)}30%{opacity:1}100%{transform:translateX(-18%) scale(0.92);opacity:0;filter:blur(4px)} }
        @keyframes chatReveal { from{opacity:0;transform:scale(0.97) translateY(10px)}to{opacity:1;transform:scale(1) translateY(0)} }
        @keyframes bookingReveal { from{opacity:0;transform:translateY(20px) scale(0.97)}to{opacity:1;transform:translateY(0) scale(1)} }
        ...
        @keyframes userBadgePop { from{opacity:0;transform:scale(0.8)}to{opacity:1;transform:scale(1)} }
        ...
        .user-badge { animation: userBadgePop 0.3s cubic-bezier(0.16,1,0.3,1) both; }

        .td { stroke-dasharray:1000;stroke-dashoffset:1000;fill:none;animation:drawStroke 0.6s ease forwards; }
        .td1{animation-delay:0.00s}.td2{animation-delay:0.10s}.td3{animation-delay:0.18s}.td4{animation-delay:0.26s}
        .td5{animation-delay:0.34s}.td6{animation-delay:0.42s}.td7{animation-delay:0.50s}.td8{animation-delay:0.58s}
        .td9{animation-delay:0.66s}.td10{animation-delay:0.74s}.td11{animation-delay:0.82s}.td12{animation-delay:0.88s}
        .tf { opacity:0;animation:fadeIn 0.3s ease forwards; }
        .tf1{animation-delay:0.05s}.tf2{animation-delay:0.14s}.tf3{animation-delay:0.22s}.tf4{animation-delay:0.30s}
        .tf5{animation-delay:0.38s}.tf6{animation-delay:0.46s}.tf7{animation-delay:0.54s}.tf8{animation-delay:0.62s}
        .tf9{animation-delay:0.70s}.tf10{animation-delay:0.78s}.tf11{animation-delay:0.85s}.tf12{animation-delay:0.91s}

        .ael-input {
          flex:1;background:#f0ede8;border:1.5px solid #d8d4ce;border-radius:12px;
          padding:11px 15px;font-size:14px;color:#1a1a1a;outline:none;
        ...
        .book-input {
          width:100%;background:#f0ede8;border:1.5px solid #d8d4ce;border-radius:10px;
          padding:10px 14px;font-size:14px;color:#1a1a1a;outline:none;
          font-family:'Nunito',sans-serif;transition:border-color 0.2s;box-sizing:border-box;
        }
        .book-input:focus { border-color:#6b6b6b; }
        .book-input::placeholder { color:#bbb; }
        .book-textarea { resize:none;height:80px; }
        .debug-toggle {
          background:none;border:1px solid #d8d4ce;border-radius:8px;
          padding:4px 8px;cursor:pointer;font-size:10px;font-family:'Nunito',sans-serif;
          color:#aaa;transition:all 0.2s;display:flex;align-items:center;gap:4px;
        }
        .debug-toggle.active { border-color:#3b82f6;color:#3b82f6;background:rgba(59,130,246,0.06); }
        .debug-toggle:hover { border-color:#999;color:#666; }
      `}</style>
            ...
            {!bookSubmitted ? (
              <>
                <p style={{ fontFamily:"'Nunito',sans-serif",fontSize:"14px",color:"#555",lineHeight:"1.6",marginBottom:"20px" }}>
                  You left our conversation — no worries. Drop your details and we'll reach out to continue.
                </p>
              ...
              <div className="success-pop" style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:"14px",padding:"20px 0",textAlign:"center" }}>
                ...
                <div style={{ fontFamily:"'Nunito',sans-serif",fontSize:"14px",color:"#777",lineHeight:"1.6" }}>
                  We'll reach out to <strong>{bookEmail}</strong> shortly.
                </div>
              </div>
            )}
          </div>
        ...
        <div className="chat-reveal fixed z-50 flex flex-col" style={{ bottom:"16px",right:"16px",width:"min(480px, calc(100vw - 32px))",height:"min(680px, calc(100vh - 120px))",borderRadius:"20px",background:"#f5f2ed",border:"1.5px solid #ddd9d2",boxShadow:"0 24px 60px rgba(0,0,0,0.14), 0 4px 16px rgba(0,0,0,0.07)",overflow:"hidden" }}>
          ...
          <div style={{ background:"#edeae4",borderBottom:"1.5px solid #ddd9d2",padding:"14px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0 }}>
            ...
            <div style={{ display:"flex",alignItems:"center",gap:"8px" }}>
              {/* User badge */}
              {currentUser && (
                <div className="user-badge" style={{ display:"flex",alignItems:"center",gap:"5px",background:"rgba(59,130,246,0.08)",border:"1px solid rgba(59,130,246,0.2)",borderRadius:"999px",padding:"3px 10px 3px 5px" }}>
                  <div style={{ width:"18px",height:"18px",borderRadius:"50%",background:"linear-gradient(135deg,#3b82f6,#6366f1)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"9px",fontWeight:800,color:"white" }}>
                    {currentUser.username.charAt(0).toUpperCase()}
                  </div>
                  <span style={{ fontSize:"11px",fontWeight:700,color:"#3b82f6",fontFamily:"'Nunito',sans-serif" }}>{currentUser.username}</span>
                </div>
              )}
              ...
              <button
                ...
                title="Toggle debug mode — see pipeline steps"
              >
                <Bug size={10} />
          ...
          <div className="ael-scroll" style={{ flex:1,overflowY:"auto",padding:"18px 14px",display:"flex",flexDirection:"column",gap:"10px",background:"#f5f2ed" }}>
            {messages.length === 0 && (
              <div style={{ display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",flex:1,gap:"12px" }}>
                ...
                <span style={{ fontFamily:"'Nunito',sans-serif",fontSize:"13px",color:"#b0aca6" }}>{getGreeting()}</span>
                {currentUser && (
                  <span style={{ fontFamily:"'Nunito',sans-serif",fontSize:"11px",color:"#c8c4bf" }}>
          ...
          <div style={{ padding:"12px 14px",borderTop:"1.5px solid #ddd9d2",background:"#edeae4",display:"flex",gap:"10px",alignItems:"center",flexShrink:0 }}>
            <input
              ...
              onKeyDown={handleKey}
              placeholder={currentUser ? `Ask anything, ${currentUser.username}...` : "Ask anything..."}
